...
export const ConnectionPanel = ({
  ...
  const tokenHelper = hasStoredToken
    ? t("Stored token available on this Studio host. Leave blank to keep it.")
    : localGatewayDefaultsHasToken
      ? t("A local OpenClaw token is available on this host. Leave blank to use it.")
      : t("Enter the token Studio should use for this upstream.");
    ...
    <div className="fade-up-delay flex flex-col gap-3">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        {onClose ? (
          <button
            ...
            aria-label={t("Close gateway connection panel")}
          >
      ...
      <div className="grid gap-3 lg:grid-cols-[1.4fr_1fr]">
        <label className="flex flex-col gap-1 font-mono text-[10px] font-semibold tracking-[0.06em] text-muted-foreground">
          {t("Upstream gateway URL")}
          <input
            className="ui-input h-10 rounded-md px-4 font-sans text-sm text-foreground outline-none"
            type="text"
            value={draftGatewayUrl}
            onChange={(event) => onGatewayUrlChange(event.target.value)}
            placeholder={locale === "zh-CN" ? "ws://localhost:18789 或 wss://your-gateway.ts.net" : "ws://localhost:18789 or wss://your-gateway.ts.net"}
            spellCheck={false}
          />
        </label>
        <label className="flex flex-col gap-1 font-mono text-[10px] font-semibold tracking-[0.06em] text-muted-foreground">
      ...
      {hasUnsavedChanges ? (
        ...
        <p className="text-xs text-muted-foreground">
          {t("Saved upstream:")} <span className="font-mono">{savedGatewayUrl || t("not configured")}</span>
        </p>
      )}
