      ...
      >
        <div className="flex items-center justify-between border-b border-border/35 px-6 py-6">
          <div>
            ...
            <div className="mt-1 text-base font-semibold text-foreground">{t("Launch agent")}</div>
            <div className="mt-1 text-xs text-muted-foreground">{t("Name it and activate immediately.")}</div>
          </div>
          <button
            type="button"
        ...
        <div className="grid gap-4 px-6 py-5">
          ...
          <div className="-mt-2 text-[11px] text-muted-foreground">
            {t("You can rename this agent from the main chat header.")}
          </div>
          <div className="grid justify-items-center gap-2 border-t border-border/40 pt-3">
            <div className={labelClassName}>{t("Choose avatar")}</div>
            <AgentAvatar
              seed={avatarSeed}
              name={name.trim() || t("New agent")}
              size={64}
              isSelected
            />
            <button
              type="button"
              aria-label={t("Shuffle avatar selection")}
        ...
        <div className="flex items-center justify-between border-t border-border/45 px-6 pb-4 pt-5">
          <div className="text-[11px] text-muted-foreground">{t("Authority can be configured after launch.")}</div>
          <button
            type="submit"
          ...
          >
            {busy ? t("Launching...") : t("Launch agent")}
          </button>
        </div>
      </form>
    </div>
