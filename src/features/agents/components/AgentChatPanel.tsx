...
const EMPTY_CHAT_INTRO_MESSAGES: Record<"en" | "zh-CN", string[]> = {
  en: [
...
const resolveEmptyChatIntroMessage = (
  ...
  const messages = EMPTY_CHAT_INTRO_MESSAGES[locale];
  if (messages.length === 0) return locale === "zh-CN" ? "今天我可以帮你做什么？" : "How can I help you today?";
  const normalizedEpoch =
    typeof sessionEpoch === "number" && Number.isFinite(sessionEpoch)
      ? Math.max(0, Math.trunc(sessionEpoch))
      : 0;
  const offset = stableStringHash(agentId) % messages.length;
  const index = (offset + normalizedEpoch) % messages.length;
...
const ExecApprovalCard = memo(function ExecApprovalCard({
  ...
  const expiryLabel = formatApprovalExpiry(approval.expiresAtMs);
  return (
    <div
    ...
    >
      <div className="type-meta">
        {locale === "zh-CN" ? "需要执行审批" : "Exec approval required"}
      </div>
      ...
      <div className="mt-2 grid gap-1 text-[11px] text-muted-foreground sm:grid-cols-2">
        <div>{locale === "zh-CN" ? "主机" : "Host"}: {approval.host ?? (locale === "zh-CN" ? "未知" : "unknown")}</div>
        <div>{locale === "zh-CN" ? "过期时间" : "Expires"}: {expiryLabel === "Unknown" && locale === "zh-CN" ? "未知" : expiryLabel}</div>
      ...
      <div className="mt-2 flex flex-wrap gap-2">
        <button
          ...
          disabled={disabled}
          aria-label={`${locale === "zh-CN" ? "单次允许执行审批" : "Allow once for exec approval"} ${approval.id}`}
        >
          {locale === "zh-CN" ? "单次允许" : "Allow once"}
        </button>
        <button
          type="button"
          className="rounded-md border border-border/70 bg-surface-3 px-2.5 py-1 font-mono text-[12px] font-medium tracking-[0.02em] text-foreground transition hover:bg-surface-2 disabled:cursor-not-allowed disabled:opacity-60"
          ...
          disabled={disabled}
          aria-label={`${locale === "zh-CN" ? "始终允许执行审批" : "Always allow for exec approval"} ${approval.id}`}
        >
          {locale === "zh-CN" ? "始终允许" : "Always allow"}
        </button>
        <button
          type="button"
          ...
          disabled={disabled}
          aria-label={`${locale === "zh-CN" ? "拒绝执行审批" : "Deny exec approval"} ${approval.id}`}
        >
            ...
            <div className="-mx-1 flex items-center justify-between gap-3 rounded-md bg-surface-2 px-3 py-2 shadow-2xs">
              <div className="type-meta min-w-0 truncate font-mono text-muted-foreground">
                {hasHiddenFetchedHistory && typeof historyFetchedCount === "number" && typeof visibleTurnCount === "number"
                  ? locale === "zh-CN"
                    ? `显示最新 ${visibleTurnCount} / ${historyFetchedCount} 条已加载轮次`
                    : `Showing latest ${visibleTurnCount} of ${historyFetchedCount} loaded turns`
                  : locale === "zh-CN"
                    ? `显示最新 ${typeof visibleTurnCount === "number" ? visibleTurnCount : "?"} 条轮次`
              ...
              {historyGatewayCapReached && !hasHiddenFetchedHistory ? (
                <div className="type-meta shrink-0 font-mono text-muted-foreground">
                  {locale === "zh-CN" ? "显示可获取的最新历史" : "Showing latest retrievable history"}
                </div>
              ) : (
                <button
                  type="button"
                ...
                >
                  {hasHiddenFetchedHistory ? t("Show older") : t("Load more")}
                </button>
              )}
            </div>
          ) : null}
...
const AgentChatComposer = memo(function AgentChatComposer({
  ...
  const stopAriaLabel = stopReason ? `${t("Stop unavailable:")} ${stopReason}` : t("Stop");
  const modelSelectedLabel = useMemo(() => {
    if (modelOptions.length === 0) return t("No models found");
    ...
    <div className="rounded-2xl border border-border/65 bg-surface-2/45 px-3 py-2">
      ...
      <div className="mt-1 flex items-center justify-between gap-2">
        <div className="flex min-w-0 items-center gap-2">
          <InlineHoverTooltip text={t("Choose model")}>
            <select
          ...
          {allowThinking ? (
            <InlineHoverTooltip text={t("Select reasoning effort")}>
              <select
        ...
        <div className="ml-auto flex items-center gap-1.5 text-[10px] text-muted-foreground">
          <span className="font-mono tracking-[0.02em]">{t("Show")}</span>
          <button
            type="button"
            role="switch"
...
export const AgentChatPanel = ({
  ...
  const emptyStateTitle = useMemo(
    () => resolveEmptyChatIntroMessage(agent.agentId, agent.sessionEpoch, locale),
    [agent.agentId, agent.sessionEpoch, locale]
  );
  ...
  const submitRename = useCallback(async () => {
    ...
    if (!nextName) {
      setRenameError(t("Agent name is required."));
      return;
    }
    ...
    try {
      ...
      if (!ok) {
        setRenameError(t("Failed to rename agent."));
        return;
      }
      setRenameEditing(false);
  ...
  }, [agent.name, onRename, renameDraft, renameSaving, t]);
      ...
      <div className="px-3 pt-2 sm:px-4 sm:pt-3">
        <div className="flex items-start justify-between gap-4">
                  ...
                  {renameEditing ? (
                    <div ref={renameEditorRef} className="flex h-8 items-center gap-1.5">
                      ...
                      <button
                        className="ui-btn-icon ui-btn-icon-sm agent-rename-control"
                        type="button"
                        aria-label={t("Cancel agent rename")}
          ...
          <div className="mt-0.5 flex items-center gap-2">
            ...
            >
              {newSessionBusy ? t("Starting...") : t("New session")}
            </button>
            <button
              className="nodrag ui-btn-icon"
              type="button"
              data-testid="agent-settings-toggle"
              aria-label={t("Open behavior")}
