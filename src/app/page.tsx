...
import { StudioI18nProvider, useStudioI18n } from "@/lib/i18n/studioI18n";
...
const AgentStudioPage = () => {
  ...
  const focusedAgentStopDisabledReason = useMemo(() => {
    ...
    return locale === "zh-CN"
      ? "该任务正在执行自动心跳检查。Studio 暂不支持停止心跳任务（即将支持）。"
  ...
  const configMutationStatusLine = activeConfigMutation
    ? locale === "zh-CN"
      ? `正在应用配置变更：${activeConfigMutation.label}`
      : `Applying config change: ${activeConfigMutation.label}`
    : queuedConfigMutationCount > 0
      ? queuedBlockedByRunningAgents
        ? locale === "zh-CN"
          ? `已排队 ${queuedConfigMutationCount} 个配置变更；等待 ${runningAgentCount} 个运行中智能体完成`
  ...
  const createBlockStatusLine = createAgentBlock
    ? createAgentBlock.phase === "queued"
      ? t("Waiting for active runs to finish")
      : createAgentBlock.phase === "creating"
      ? t("Submitting config change")
      : null
    : null;
  ...
  const restartingMutationAriaLabel = restartingMutationBlock
    ? restartingMutationBlock.kind === "delete-agent"
      ? t("Deleting agent and restarting gateway")
      : t("Renaming agent and restarting gateway")
    : null;
  const restartingMutationHeading = restartingMutationBlock
    ? restartingMutationBlock.kind === "delete-agent"
      ? t("Agent delete in progress")
      : t("Agent rename in progress")
    : null;
        ...
        <div className="flex min-h-screen items-center justify-center px-6">
          <div className="glass-panel ui-panel w-full max-w-md px-6 py-6 text-center">
            ...
            <div className="mt-3 text-sm text-muted-foreground">
              {gatewayConnecting ? t("Connecting to gateway…") : t("Booting Studio…")}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="relative min-h-screen w-screen overflow-hidden bg-background">
        <div className="flex min-h-screen items-center justify-center px-6">
          <div className="glass-panel ui-panel w-full max-w-md px-6 py-6 text-center">
            ...
            <div className="mt-3 text-sm text-muted-foreground">{t("Loading agents…")}</div>
          </div>
        </div>
      </div>
    );
            ...
            >
              <aside className="w-[240px] shrink-0 border-r border-border/60">
                ...
                <nav className="py-3">
                  {(
                    [
                      { id: "personality", label: t("Behavior") },
                      { id: "capabilities", label: t("Capabilities") },
                      { id: "automations", label: t("Automations") },
                ...
                <div className="flex items-start justify-between border-b border-border/60 px-6 py-4">
                  <div>
                    <div className="text-lg font-semibold text-foreground">
                      {inspectSidebarAgent?.name ?? settingsRouteAgentId ?? t("Agent settings")}
                    </div>
                    <div className="mt-1 font-mono text-[11px] text-muted-foreground">
                      {t("Model")}: {settingsHeaderModel}{" "}
                      <span className="mx-2 text-border">|</span>
                      {t("Thinking")}: {settingsHeaderThinking}
                    </div>
                  </div>
                  <div className="rounded-md border border-border/70 bg-surface-1 px-3 py-1 font-mono text-[11px] text-muted-foreground">
                    [{personalityHasUnsavedChanges ? t("Unsaved") : t("Saved ✓")}]
                  </div>
                </div>
                <div className="min-h-0 flex-1 overflow-hidden">
                  {inspectSidebarAgent ? (
                    ...
                    <EmptyStatePanel
                      ...
                      description={t("Back to chat and select an available agent.")}
                      fillHeight
            ...
            <div className="flex min-h-0 flex-1 flex-col gap-4 xl:flex-row">
              ...
              >
                {focusedAgent ? (
                  ...
                  <EmptyStatePanel
                    title={hasAnyAgents ? t("No agents match this filter.") : t("No agents available.")}
                    description={
                      hasAnyAgents
                        ? undefined
                        : gatewayConnected
                          ? t("Use New Agent in the sidebar to add your first agent.")
                          : t("Connect to your gateway to load agents into the studio.")
                    }
                    fillHeight
                    className="items-center p-6 text-center text-sm"
                  />
        ...
        >
          <div className="ui-panel w-full max-w-md p-6">
            <div className="font-mono text-[10px] font-semibold tracking-[0.06em] text-muted-foreground">
              {t("Agent create in progress")}
            </div>
            <div className="mt-2 text-base font-semibold text-foreground">
              {createAgentBlock.agentName}
            </div>
            <div className="mt-3 text-sm text-muted-foreground">
              {t("Studio is temporarily locked until creation finishes.")}
            </div>
      ...
      {restartingMutationBlock && restartingMutationBlock.phase !== "queued" ? (
        ...
        >
          <div className="ui-panel w-full max-w-md p-6">
            ...
            <div className="mt-3 text-sm text-muted-foreground">
              {t("Studio is temporarily locked until the gateway restarts.")}
            </div>
